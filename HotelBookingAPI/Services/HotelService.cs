...
namespace HotelBookingAPI.Services
{
    public class HotelService : IHotelService
    {
        ...
        //{
        //    var hotels = _dbContext.Hotels.Include(a=> a.HotelAddress).ToList();
        ...
        //        HotelFacilities = _dbContext.HotelFacilities.Include(a=>a.FacilityDetails).Where(i => i.HotelId == x.HotelId).ToList(),
        //        HotelRooms = _dbContext.HotelRooms.Include(a=> a.RoomDetails).Where(i => i.HotelId == x.HotelId).ToList(),
        ...
        {
            var hotels = string.IsNullOrEmpty(hotelName) ? _dbContext.Hotels.Include(a => a.HotelAddress).ToList() :
                _dbContext.Hotels.Where(x=>x.HotelName.Trim().ToLower().Contains(hotelName.Trim().ToLower())).Include(a => a.HotelAddress).ToList();
            var hoteldetails = new List<HotelDetails>();
            hotels.ForEach(x => hoteldetails.Add(new HotelDetails
            {
                HotelDetail = x,
            ...
            {
                var cityId = _dbContext.Cities.Where(x => x.CityName.ToLower().Equals(cityName.ToLower())).Select(x => x.CityId).FirstOrDefault();
        ...
        {
            var hotel = _dbContext.Hotels.Where(x=>x.HotelId == hotelId).Include(a => a.HotelAddress).FirstOrDefault();
            var hoteldetail = new HotelDetails
            {
                ...
                HotelRooms = _dbContext.HotelRooms.Include(a => a.RoomDetails).Where(i => i.HotelId == hotel.HotelId).ToList(),
                HotelFacilities = _dbContext.HotelFacilities.Include(a => a.FacilityDetails).Where(i => i.HotelId == hotel.HotelId).ToList(),
